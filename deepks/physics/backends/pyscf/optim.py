...
from deepks.physics.backends.pyscf.scf import t_make_eig, t_make_grad_eig_dm
...
def make_grad_eig_egrad(dscf, mo_coeff=None, mo_occ=None, gfock=None):
    ...
    if mo_coeff.ndim >= 3 and mo_occ.ndim >= 2:
        return np.concatenate([make_grad_eig_egrad(dscf, mc, mo, gfock) for mc, mo in zip(mo_coeff, mo_occ)], axis=-1)
    iocc = mo_occ > 0
...
def calc_optim_veig(dscf, target_dm, target_dec=None, gvx=None, nstep=1, force_factor=1.0, **optim_args):
    ...
    def closure():
        [t_vc] = torch.autograd.grad(t_eig, t_dm, t_veig, retain_graph=True, create_graph=True)
        loss, dldv = clfn(t_vc.detach().numpy(), target_dm)
        grad = torch.autograd.grad(t_vc, t_veig, torch.from_numpy(dldv), only_inputs=True)[0]
...
def gcalc_optim_veig(gdscf, target_dm, target_grad, nstep=1, force_factor=1.0, **optim_args):
