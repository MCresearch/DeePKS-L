...
from deepks.config.packager import get_packed_payload, get_payload_key, is_packed_config
...
def resolve_scf_profile_levels(config):
    """Resolve per-basis SCF levels from ``physics.backend.profiles``.

    Unlike :func:`resolve_hierarchical_iterate_levels` (driven by the additive-
    stack model's ``levels``), this is driven purely by the declared SCF
    ``profiles`` and works for ANY recipe. One SCF level is produced per
    profile; ``data.train`` / ``data.test`` are FLAT lists aligned 1:1 to the
    profiles by index. This lets a single shared model (e.g. ``corrnet``) run
    per-basis SCF as a Sequence and have its per-level outputs pooled into one
    training set. Returns ``[]`` for the hierarchical recipe (handled by the
    dedicated resolver) or when no profiles are declared.
    """
    ...
    if train_groups and len(train_groups) != len(raw_profiles):
        raise ValueError(
            "physics.backend.profiles and data.train must align 1:1 for per-basis SCF: "
            f"got {len(raw_profiles)} profiles but {len(train_groups)} data.train entries"
        )
    ...
    for index, raw_profile in enumerate(raw_profiles):
        ...
        profile = deepcopy(raw_profile)
        profile.setdefault("level", index)
        profile.setdefault("name", f"level_{index}")
        override_backend_input = profile.get("input") if isinstance(profile.get("input"), dict) else {}
        systems = {"level": index}
        if index < len(train_groups):
            systems["train_paths"] = _normalize_group(train_groups[index], f"data.train[{index}]")
        if index < len(test_groups) and test_groups[index] is not None:
            systems["test_paths"] = _normalize_group(test_groups[index], f"data.test[{index}]")
        resolved.append({
            "level": index,
            "profile": profile,
            "merged_backend_input": _merge_nested(global_backend_input, override_backend_input),
            "systems": systems,
        })
    return resolved
