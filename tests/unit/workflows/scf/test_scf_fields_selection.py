"""
整体覆盖：PySCF 接口侧 `deepks.physics.backends.pyscf.fields` 的字段选择与单位辅助函数。
...
def test_select_fields_by_name_and_alias():
	"""
	依赖：`deepks.physics.backends.pyscf.fields.select_fields`。
...
def test_isinbohr_and_lunit():
	"""
	依赖：`deepks.physics.backends.pyscf.fields.isinbohr/_Lunit`。
...
def test_atom_data_filters_ghost_atoms():
	"""
	依赖：`deepks.physics.backends.pyscf.fields.atom_data`。
