...
from deepks.scf.penalty import CoulombPenalty, DensityPenalty, select_penalty


def test_select_penalty_basic():
	"""
	依赖：`deepks.scf.penalty.select_penalty`。
...
def test_density_penalty_init_and_defaults(tmp_path):
	"""
	依赖：`deepks.scf.penalty.DensityPenalty`。
...
def test_coulomb_penalty_init_and_defaults(tmp_path):
	"""
	依赖：`deepks.scf.penalty.CoulombPenalty`。
